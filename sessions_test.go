...
func TestParseQuackActiveConnections(t *testing.T) {
	...
	// Row 3: a state the table needs a glyph for.
	if conns[2].Status != "cancelled" {
		t.Errorf("Status = %q, want cancelled", conns[2].Status)
	}
	if statusGlyph("cancelled") == "  " {
		t.Error("cancelled sessions should get their own glyph")
	}
	if statusGlyph("finished") == "  " {
		t.Error("finished sessions should get their own glyph")
	}
}
