...
func NewQuackClient(cfg ServerConfig, resolver ConfigResolver, secretResolver SecretResolver, opts ...ClientOption) *QuackClient {
	if resolver == nil {
...
//
// These exist because connection state is otherwise discovered — by Ping, and by
// looking for duckdb on PATH — and a caller that already knows the answer has no
// way to say so. Tests are the main user: asserting on a screen's behaviour when
// a connection is online should not require a live backend, and asserting on
// what gets sent should not require duckdb to be installed.
type ClientOption func(*QuackClient)

// WithState fixes the connection state instead of discovering it with Ping.
func WithState(s ConnState) ClientOption {
	return func(c *QuackClient) {
		c.mu.Lock()
		defer c.mu.Unlock()
		c.state = s
	}
...
// WithCLI overrides duckdb discovery, naming the binary to run. Pass "" to
// declare no CLI available.
func WithCLI(path string) ClientOption {
	return func(c *QuackClient) {
		c.cliPath = path
		c.hasCLI = path != ""
	}
...
// Sessions lists what is currently connected, by backend type:
//   - Quack    — the server's own quack_active_connections().
//   - Local    — a single "local" session (us).
//   - DuckLake — the most recent snapshots as pseudo-sessions.
//
// reportedCount is the count the backend gave us, if any. It is returned
// separately from the listing because DuckDB exposes a count but not always a
// per-connection listing — two different facts.
func (c *QuackClient) Sessions(ctx context.Context) (conns []Connection, reportedCount string, err error) {
	if !c.GetState().Online || !c.hasCLI {
		return nil, "", fmt.Errorf("offline or no CLI")
	}

	// Real sessions only exist on backends that expose them over a network. For
	// others we synthesise something useful per type so the panel isn't empty.
	if c.Config.Supports(CapSessions) {
		return c.quackSessions(ctx)
	}

	switch c.Config.Type {
	case ConnDuckLake:
		return c.duckLakeSnapshotSessions(ctx)
	case ConnLocal:
		return []Connection{{
	...
	return nil, "", fmt.Errorf("no sessions for %s", c.Config.Type)
}
...
func (c *QuackClient) quackSessions(ctx context.Context) ([]Connection, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("session query: %s", cliError(err))
	}
	return parseSessionRows(out, c.Config)
}
...
func (c *QuackClient) duckLakeSnapshotSessions(ctx context.Context) ([]Connection, string, error) {
	...
	if err != nil {
		...
		return nil, "", fmt.Errorf("snapshot query: %s", cliError(err))
	}
...
// Catalog lists the relations on whichever backend this client points at —
// works uniformly across Quack, Local, and DuckLake.
func (c *QuackClient) Catalog(ctx context.Context) ([]CatalogSchema, error) {
	if !c.GetState().Online || !c.hasCLI {
		return nil, fmt.Errorf("offline or no CLI")
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("catalog query: %s", cliError(err))
	}
	return parseCatalogRows(out)
}
...
// CopyToParquet re-runs sql on the backend and writes the result to path as
// Parquet. It lives here rather than in the screen that offers the export
// because it needs the duckdb invocation machinery — reaching into that from the
// UI is what the package split exists to stop.
func (c *QuackClient) CopyToParquet(ctx context.Context, sql, path string) error {
	if !c.hasCLI {
		return fmt.Errorf("duckdb CLI not found in PATH")
	}
