...
func TestMetadataQueriesAgainstRealDuckDB(t *testing.T) {
	...
	t.Run("catalog", func(t *testing.T) {
		schemas, err := c.Catalog(context.Background())
		if err != nil {
			t.Fatalf("catalog fetch failed: %v", err)
		}
	...
	t.Run("queries surface real errors", func(t *testing.T) {
		res := c.Query(context.Background(), "SELECT no_such_column FROM analytics.orders;")
		if res.Err == "" {
		...
		if !strings.Contains(res.Err, "no_such_column") {
			t.Errorf("Err = %q, want it to name the offending column", res.Err)
		}
		if strings.Contains(res.Err, "no endpoint responded") {
			t.Errorf("Err = %q, HTTP fallback should not apply to local connections", res.Err)
		}
	})
	...
	t.Run("successful query returns rows", func(t *testing.T) {
		res := c.Query(context.Background(), "SELECT id FROM analytics.orders ORDER BY id LIMIT 3;")
		if res.Err != "" {
		...
		if res.Method != "cli" {
			t.Errorf("method = %q, want cli", res.Method)
		}
		if res.Rows[0][0] != "0" {
			t.Errorf("first row = %v, want id 0", res.Rows[0])
		}
	})
