...
type Scratchpad struct {
	...
	servers   []quack.ServerInfo
	serverIdx int

	clients []*quack.QuackClient // may be nil entries or nil slice when unconfigured
...
func NewScratchpad(servers []quack.ServerInfo, clients []*quack.QuackClient) Scratchpad {
	ta := textarea.New()
...
func (sp *Scratchpad) SetTargets(servers []quack.ServerInfo, clients []*quack.QuackClient) {
	sp.servers = servers
	sp.clients = clients
...
func (sp Scratchpad) target() (quack.ServerInfo, bool) {
	if sp.serverIdx < 0 || sp.serverIdx >= len(sp.servers) {
		return quack.ServerInfo{}, false
	}
...
func (sp Scratchpad) runQuery() (Scratchpad, tea.Cmd) {
	...
	if _, ok := sp.target(); !ok {
		return sp, func() tea.Msg {
			return queryResultMsg{result: &quack.QueryResult{
				Query:     sql,
	...
	return sp, queryCmd(client, ctx, sql)
}
...
func (sp Scratchpad) ViewResultsStatus() string {
	...
	if sp.running {
		return "  " + amberStyle.Render("⟳ running…") +
			mutedStyle.Render("  ctrl+c or esc to interrupt  ·  deadline "+quack.QueryTimeout().String())
	}
	if sp.result == nil {
	...
	if sp.clients != nil && sp.serverIdx < len(sp.clients) && sp.clients[sp.serverIdx] != nil {
		...
		case !c.HasCLI() && c.Config.Type != quack.ConnQuack:
...
func renderResultTable(r quack.QueryResult, maxWidth int) string {
	if r.Err != "" {
...
func exportCSV(r quack.QueryResult) (string, error) {
...
func exportParquet(c *quack.QuackClient, sql string) (string, error) {
	if c == nil || !c.HasCLI() {
	...
	if err := c.CopyToParquet(ctx, sql, path); err != nil {
		return "", err
	}
	return path, nil
}
