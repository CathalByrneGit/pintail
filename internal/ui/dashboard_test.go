...
func threeConnModel(t *testing.T) Model {
	t.Helper()
	configs := []quack.ServerConfig{
		{Name: "central", Type: quack.ConnQuack, Host: "a", Port: 9494},
		{Name: "lake-prod", Type: quack.ConnDuckLake, CatalogPath: "/tmp/c.duckdb", StoragePath: "/tmp/d"},
		{Name: "local-dev", Type: quack.ConnLocal, Path: "/tmp/x.duckdb"},
	}
	m := Model{
		configs:     configs,
		clients:     quack.InitClients(configs, nil),
...
func sessionsFor(names ...string) []quack.Connection {
	out := make([]quack.Connection, 0, len(names))
	for _, n := range names {
		out = append(out, quack.Connection{ID: n, Identity: n, Status: "active"})
	}
	return out
}
...
func TestCatalogResultsAndErrorsAreStoredPerConnection(t *testing.T) {
	...
	good := []quack.CatalogSchema{{Name: "analytics", Tables: []quack.CatalogTable{{Name: "orders"}}}}
...
func TestResultsForUnknownConnectionsAreDropped(t *testing.T) {
	...
	for _, idx := range []int{-1, 3, 99} {
		...
		m = next.(Model)
		next, _ = m.Update(catalogResultMsg{idx: idx, catalog: []quack.CatalogSchema{{Name: "ghost"}}})
		m = next.(Model)
	}
...
func TestDashboardPanelsNameTheirConnection(t *testing.T) {
	...
	m.data[1].catalog = []quack.CatalogSchema{{Name: "analytics", Open: true,
		Tables: []quack.CatalogTable{{Name: "orders", Format: "table", Rows: 10, SizeKnown: true}}}}
...
func TestAddingAConnectionExtendsData(t *testing.T) {
	...
	m.addForm = newAddServerForm()
	m.addForm.connType = quack.ConnLocal
...
func TestDashboardRendersAtAnySize(t *testing.T) {
	...
	populated.data[2].catalog = []quack.CatalogSchema{{Name: "main", Open: true,
		Tables: []quack.CatalogTable{{Name: "events", Format: "table", Rows: 100, SizeKnown: true}}}}
...
// The states quack_active_connections() can report, from the extension's
// QueryStateToString: idle, active, finished, cancelled, and unknown for
// anything it does not recognise. Each needs to be distinguishable in the table
// rather than sharing the blank default — "unknown" included, since that is what
// a state added in a later Quack will arrive as.
func TestStatusGlyphCoversTheReportedStates(t *testing.T) {
	seen := map[string]string{}
	for _, state := range []string{"idle", "active", "finished", "cancelled", "unknown"} {
		got := statusGlyph(state)
		if got == "  " {
			t.Errorf("state %q has no glyph of its own", state)
		}
		if prev, dup := seen[got]; dup {
			t.Errorf("states %q and %q share the glyph %q", prev, state, got)
		}
		seen[got] = state
	}
}

// The dashboard shows the running query for the highlighted row, since it is too
// long for a table column.
func TestFooterShowsSelectedSessionQuery(t *testing.T) {
	m := threeConnModel(t)
	m.data[0].sessions = []quack.Connection{
		{ID: "1", Identity: "srv-a", Status: "active", Query: "SELECT count(*) FROM orders WHERE ts > now() - INTERVAL 1 DAY", Duration: time.Second},
		{ID: "2", Identity: "srv-a", Status: "idle"},
	}
	m.selectConnection(0)
	m.focus = panelConnections

	footer := m.viewDashboardFooter()
	if !strings.Contains(footer, "SELECT count(*)") {
		t.Errorf("footer should show the running query for the selected row:\n%s", footer)
	}

	// Move to the idle session: nothing to show, and no stale query left behind.
	m.connTable.SetCursor(1)
	footer = m.viewDashboardFooter()
	if strings.Contains(footer, "SELECT count(*)") {
		t.Errorf("footer still shows the previous row's query:\n%s", footer)
	}
}
