...
func newSecretForm() *secretForm {
	return &secretForm{sectype: quack.SecretS3, focusIdx: -1, editing: -1}
}

func secretFormFromExisting(s quack.StorageSecret, idx int) *secretForm {
	return &secretForm{
		sectype:   s.Type,
		focusIdx:  0,
		editing:   idx,
...
func (f *secretForm) toSecret() quack.StorageSecret {
	return quack.StorageSecret{
...
func (tm TokenManager) updateSecretForm(msg tea.KeyMsg) (TokenManager, tea.Cmd) {
	...
	case "left":
		if f.focusIdx == -1 {
			for i := 0; i < len(quack.AllSecretTypes)-1; i++ {
	...
	case "enter":
		...
		tm.successTTL = 60
		_ = quack.SaveStorageSecrets(tm.secrets)
		tm.secretForm = nil
		return tm, nil
...
func (tm *TokenManager) selectedSecret() *quack.StorageSecret {
...
func (tm TokenManager) ViewSecretDetail(width int) string {
	...
	default:
		...
		if s.Type == quack.SecretS3 && s.Region != "" {
			rows = append(rows, row("region", brightStyle.Render(s.Region)))
		}
		if s.Type == quack.SecretR2 {
...
func (tm TokenManager) ViewSecretForm(width, height int) string {
	...
	// Type selector
	var chips []string
	for _, t := range quack.AllSecretTypes {
...
func exportSecretSQL(s quack.StorageSecret) (string, error) {
