...
func TestViewsSurviveNarrowWidths(t *testing.T) {
	...
	tok := quack.BuildToken("etl_pipeline_prod", "analytics, raw", "SELECT, INSERT", "never")
	revoked := quack.BuildToken("old_token", "analytics", "SELECT", "never")
	revoked.Active = false

	secret := quack.StorageSecret{
		Name: "lake_s3", Type: quack.SecretS3,
		KeyID: "AKIAEXAMPLE", Secret: "supersecret",
		Region: "us-east-1", Scope: "s3://datalake-prod/lake",
	}

	lakeCfg := quack.ServerConfig{
		Name: "lake-prod", Type: quack.ConnDuckLake,
		CatalogPath: "/tmp/lake/catalog.duckdb", StoragePath: "/tmp/lake/data",
	}

	renders := map[string]func(w int){
		"ViewTokenList": func(w int) {
			tm := TokenManager{tokens: []quack.Token{tok, revoked}}
		...
		"ViewSecretList": func(w int) {
			tm := TokenManager{mode: tmModeSecrets, secrets: []quack.StorageSecret{secret}}
		...
		"ViewPolicyList": func(w int) {
			_ = NewAuthEditor([]quack.Token{tok, revoked}, nil).ViewPolicyList(w, 20)
		},
		"ViewPermGrid": func(w int) {
			_ = NewAuthEditor([]quack.Token{tok}, nil).ViewPermGrid(w)
		},
		"ViewPermGrid/noPolicies": func(w int) {
			_ = NewAuthEditor(nil, nil).ViewPermGrid(w)
		},
		"TLSGenerator.ViewForm": func(w int) {
			_ = NewTLSGenerator([]quack.ServerConfig{{Name: "q", Host: "localhost", Port: 9494}}).ViewForm(w)
		},
...
func TestViewTokenListAtEightyColumns(t *testing.T) {
	tm := TokenManager{tokens: []quack.Token{
		quack.BuildToken("etl_pipeline_prod", "analytics, raw, staging", "SELECT", "never"),
	}}
...
func newTestSnapshotsView(cfg quack.ServerConfig) SnapshotsView {
	v := NewSnapshotsView([]*quack.QuackClient{quack.NewQuackClient(cfg, nil, nil)})
	v.snapshots = []quack.Snapshot{{
