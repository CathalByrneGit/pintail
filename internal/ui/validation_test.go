...
func TestFormValidation(t *testing.T) {
	existing := []quack.ServerConfig{
		{Name: "central", Type: quack.ConnQuack, Host: "a", Port: 9494},
		{Name: "local-dev", Type: quack.ConnLocal, Path: "/tmp/x.duckdb"},
	}
	secrets := []quack.StorageSecret{{Name: "lake_s3", Type: quack.SecretS3, KeyID: "k", Secret: "s"}}
	...
	tests := []struct {
		...
		},
		{
			...
			form: func() *addServerForm {
				f := newAddServerForm()
				f.connType, f.name = quack.ConnLocal, "files"
				return f
			},
		...
		},
		{
			...
			form: func() *addServerForm {
				f := newAddServerForm()
				f.connType, f.name, f.storagePath = quack.ConnDuckLake, "lake", "/tmp/data"
				return f
			},
		...
		},
		{
			...
			form: func() *addServerForm {
				f := newAddServerForm()
				f.connType, f.name, f.catalogRef = quack.ConnDuckLake, "lake", "central"
				return f
			},
		...
		},
		{
			...
			form: func() *addServerForm {
				f := newAddServerForm()
				f.connType, f.name = quack.ConnDuckLake, "lake"
		...
		},
		{
			...
			form: func() *addServerForm {
				f := newAddServerForm()
				f.connType, f.name, f.path = quack.ConnLocal, "files", "/tmp/a.duckdb"
				f.secretRef = "nope"
				return f
			},
...
func TestSaveRefusedShowsWhy(t *testing.T) {
	...
	configs := []quack.ServerConfig{{Name: "central", Type: quack.ConnQuack, Host: "a", Port: 9494}}
	m := Model{
		configs:     configs,
